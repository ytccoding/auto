...
#1.4移除不必要import,修改error輸出判斷,修改讀取的EXCEL
...
for i in range(test_web.webPageSelect(webPageSelect)): #所有分頁
    ...
    for j in range(len(test_web.webPlay())): #該分頁所有可點選玩法都點
        ...
        if test_web.webPlay()[j].text in boxList1:
            test_web.elementsClickAll("ClickShade" ,2 ,len(test_web.elements("a[class='ClickShade']",6)))
            submitCheck()
        elif test_web.webPlay()[j].text in ballList1:
            for m in range(len(test_web.webBall())):
                   test_web.webBallClick(m)
            submitCheck()
        elif test_web.webPlay()[j].text in ballList2:
            clickList = [[[0,1,2,3,4,5,6,7,8]],\
            ...
            for m in range(len(clickList)):
                ...
                test_web.elementClick("a[class='betBtn ClickShade']" ,6)
            submitCheck()
        else:
...
if len(error) != 1 and error[1] != None:
    Error = open(os.getcwd() + "\\" + str(testdayFile) + "\\" + str(testdayTime) + "_QQ分分彩_ERROR.txt" ,"wb+")
    for i in error:
